# Project Backlog

## Sprint 1: Project Scaffolding & Hello World (Done)
- [x] **WK-001: Project Initialization** - Initialize Vite + React + TS with `vite-plugin-monkey`.
- [x] **WK-002: Basic UI Injection** - Inject a simple panel into the WaniKani dashboard.
- [x] **WK-003: Shadow DOM Setup** - (Trialed, moved to Sibling DOM in v0.0.2 for compatibility).
- [x] **WK-004: Repository Setup** - Finalize the folder structure and initial Git commit.

## Sprint 2: Core API & Visual Delivery (Current)
- [x] **WK-005: WaniKani API Connection** - Integrate WaniKani API to fetch learned progress.
- [x] **WK-006: Gemini API Integration** - Connect to Gemini for personalized lesson generation.
- [x] **WK-007: UI Design & Lesson Manager** - Build the core React components for settings and lesson display.
- [x] **WK-008: Local Storage & Logging** - Implement persistent settings for API keys.
- [x] **WK-008.1: Visual Verification** - Achieve unmissable visual delivery on the WaniKani dashboard.

## Future Sprints (Planned)
- [ ] **WK-009: WKOF Integration (Priority)** - Integrate WaniKani Open Framework for high-performance subject caching.
- [ ] **WK-010: Advanced Prompt Engineering** - Refine Gemini prompts for Japanese grammar practice.
- [ ] **WK-011: Testing Suite** - Set up Vitest and write initial unit tests.
- [ ] **WK-012: Export to Anki** - Add feature to export generated sentences to Anki cards.
- [ ] **WK-013: UI Refinement** - Beautify the "ugly box" into a native WaniKani-looking widget.
